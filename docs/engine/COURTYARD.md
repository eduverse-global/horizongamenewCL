...
- Reflective river (Cinematic mode): three.js `Reflector` (vendored in `dist/vendor/addons/objects/`) renders the mirrored scene at 1024², rippled and added at reduced strength to a deep green base, so the bright sky becomes a sheen and nearby posts and trees show as shapes. Performance mode keeps the plain animated water. The ground now ends at the stone bank so the river is visible, and the reflection adds about 25 draw calls.
