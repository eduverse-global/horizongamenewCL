...
- NPCs stand still at rest (their PixelLab breathing loops stay in the atlas for cutscenes and events) and turn to face the captain from any of 8 directions while talking, turning back 1.2 s after the conversation ends.
- Layered portraits for all three speakers, colour-graded to the scene: a cut-out character (`dist/assets/portraits/<id>.png`, or `<id>-evening.png` at dusk) over a backdrop for the current location and time of day (`bg-courtyard-day.jpg`, `bg-courtyard-evening.jpg`), generated by `scripts/gemini-layered.py`.
