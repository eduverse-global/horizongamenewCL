...
addEventListener('keydown',e=>{if(blocked()||e.target.closest('button,a,summary'))return;const k=e.key.toLowerCase();if(k==='='||k==='+'||k==='-'){zoom=Math.min(1,Math.max(0,zoom+(k==='-'?-.25:.25)));return;}if(['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright',' ','e'].includes(k)){e.preventDefault();if(!e.repeat){if(k==='e')interact();else if(k===' ')strike();}keys.add(k);}});
