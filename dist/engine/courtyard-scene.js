...
export async function createCourtyard(scene){
 ...
 function mesh(g,m,x,y,z){const o=new THREE.Mesh(g,m);o.position.set(x,y,z);o.castShadow=true;o.receiveShadow=true;scene.add(o);return o;}
 ...
 const glow=new THREE.MeshStandardMaterial({color:'#ffe6ad',emissive:'#ffb54d',emissiveIntensity:2.5,roughness:.5});const lamps=[];
 function lantern(x,z,y=2.35){box(x,y/2,z,.095,y,.095,dark);box(x,y+.14,z,.44,.53,.42,dark);box(x,y+.14,z,.34,.42,.43,glow);box(x,y+.14,z,.45,.42,.31,glow);
 for(const dx of[-.22,.22])for(const dz of[-.22,.22])box(x+dx,y+.14,z+dz,.035,.58,.035,brass);
 const cap=mesh(new THREE.ConeGeometry(.42,.25,4),dark,x,y+.55,z);cap.rotation.y=Math.PI/4;
 const light=new THREE.PointLight('#ffc17a',0,8,2);light.position.set(x,y+.2,z);scene.add(light);lamps.push(light);}
 for(const [x,z]of[[-2.3,-2.3],[2.3,-2.3],[7.8,2.1],[-6.3,1.8],[-2.8,7.8]])lantern(x,z);
 ...
 const target=mesh(new THREE.CylinderGeometry(.38,.38,.23,20),mat('#b79559'),5.5,1.38,4.5);target.rotation.x=Math.PI/2;
 ...
 const river=mesh(new THREE.PlaneGeometry(20,80,1,1),water,20,-.28,-10);river.rotation.x=-Math.PI/2;river.castShadow=false;
 ...
 const bladeGeo=new THREE.BufferGeometry();bladeGeo.setAttribute('position',new THREE.Float32BufferAttribute([-.05,0,0,.05,0,0,.04,.35,.06],3));bladeGeo.computeVertexNormals();const bladeMat=mat('#688a46',{side:THREE.DoubleSide});
